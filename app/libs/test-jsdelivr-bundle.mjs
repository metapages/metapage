...
function checkDistExists() {
  ...
  if (stats.size < 10000) {
    fail(`dist/index.js is suspiciously small (${stats.size} bytes). Build may have failed.`);
    return false;
  }
...
function checkNpmPack() {
  ...
  try {
    ...
    if (distFileCount === 0) {
      fail('npm pack does NOT include any dist/ files! Check .npmignore and files field in package.json.');
      console.log('   Hint: Make sure dist/ is built and not excluded by .npmignore');
      return false;
    }

    if (distFileCount < 5) {
      fail(`npm pack only includes ${distFileCount} dist/ files. Expected many more.`);
      return false;
    }

    pass(`npm pack includes ${distFileCount} dist/ files (${totalFiles ? totalFiles[1] : '?'} total files)`);
    return true;
  } catch (error) {
...
async function main() {
  ...
  console.log('   This ensures the package will work on jsDelivr and other CDNs.\n');
