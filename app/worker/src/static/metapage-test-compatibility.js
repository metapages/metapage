...
TESTS = [
    ...
    (metapage) => { // metapage instance
        return new Promise((resolve, reject) => {
            ...
            const onStateChange = (e) => {
                ...
                console.log(`🍉🍉🍉 onStateChange: ${JSON.stringify(e, null, 2)}`);
    ...
    new Test(
        `all metaframe${enablePluginTests ? "+plugins" : ""} report tests pass`,
            ...
            return new Promise((resolve, reject) => {
                try {
                    ...
                    const onStateChange = (metapageState) => {
                        ...
                        const pluginIds = enablePluginTests
                        ...
                        Object.keys(metapageState.metaframes.outputs).forEach(
                            (metaframeId) => {
                                if (
                                    ...
                                    metaframeAndPluginIdsPassed[metaframeId] =
                                        true;
                                }
                            },
                        );
                        if (enablePluginTests) {
                            Object.keys(metapageState.plugins.outputs).forEach(
                                (pluginId) => {
                                    if (
                                        metapageState.plugins
                                            .outputs[pluginId] &&
                                        metapageState.plugins.outputs[pluginId]
                                                .tests === "pass"
                                    ) {
                                        metaframeAndPluginIdsPassed[pluginId] =
                                            true;
                                    }
                                },
                            );
                        ...
                        const idsPassed = Object.keys(
                            metaframeAndPluginIdsPassed,
                        );
                        idsPassed.sort();
                        if (
                            idsPassed.length ===
                                metapageIds.length + pluginIds.length
                        ) {
                            disposeListener();
                            resolve(true);
                            setStatus();
...
if (enablePluginTests) {
    metaPageDefinition.plugins = VERSIONS_METAFRAME
        .filter((v) =>
            v.startsWith("latest") || globalThis.compareVersions(v, "0.3") >= 0
        )
...
if (Metapage.DEFINITION_UPDATE_REQUEST) {
    metapageInstance.addListener(
        Metapage.DEFINITION_UPDATE_REQUEST,
        (newDefinition) => {
...
var pluginsIds = metaPageDefinition.plugins
    ? metaPageDefinition.plugins.concat([])
    : [];
// Add the metaframe (and plugin) iframes to the page
...
for (const metaframeId of metaframeIds) {
    ...
    if (enablePluginTests && pluginsIds.length > 0) {
