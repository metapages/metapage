...
export const getMetapageImportUrl = (
  ...
  if (version === "latest") {
    ...
    if (compare(parse(versionForUrl), parse("0.16.0")) >= 0) {
      metapageScriptSrc =
        `https://cdn.jsdelivr.net/npm/@metapages/metapage@${versionForUrl}`;
    } else if (compare(parse(versionForUrl), parse("0.11.0")) >= 0) {
      metapageScriptSrc =
        `https://cdn.jsdelivr.net/npm/@metapages/metapage@${versionForUrl}/dist/browser/metaframe/index.js`;
    } else if (compare(parse(versionForUrl), parse("0.8.0")) >= 0) {
      metapageScriptSrc =
        `https://cdn.jsdelivr.net/npm/@metapages/metapage@${versionForUrl}/browser/metaframe/index.js`;
    } else if (compare(parse(versionForUrl), parse("0.5.5")) === 0) {
      metapageScriptSrc =
        `https://cdn.jsdelivr.net/npm/@metapages/metapage-backup@${versionForUrl}/browser/metaframe/index.js`;
    } else if (compare(parse(versionForUrl), parse("0.5.0")) >= 0) {
      metapageScriptSrc =
        `https://cdn.jsdelivr.net/npm/@metapages/metapage@${versionForUrl}/browser/metaframe/index.js`;
    } else if (compare(parse(versionForUrl), parse("0.4.100")) >= 0) {
      metapageScriptSrc =
        `https://cdn.jsdelivr.net/npm/metaframe@${versionForUrl}/browser/index.js`;
    } else {
      metapageScriptSrc =
        `https://cdn.jsdelivr.net/npm/metaframe@${versionForUrl}/browser${
          debug ? "" : ".min"
        }.js`;
    }
  }
