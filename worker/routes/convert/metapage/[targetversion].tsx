...
export const handler = define.handlers({
  ...
  async POST(ctx) {
    ...
    if (contentType && contentType.includes("application/json")) {
      ...
      try {
        if (form.get("metapageDefinition")) {
          console.log(
            'form.get("metapageDefinition")',
            form.get("metapageDefinition"),
          );
          console.log("form", form);
          metapageDefinition = JSON.parse(
            form.get("metapageDefinition") as string | null || "",
          );
        }
      } catch {
        return new Response("Invalid JSON", { status: 400 });
    ...
    try {
      const newVersion = await convertMetapageDefinitionToVersion(
        metapageDefinition,
        targetVersion as VersionsMetapage,
      );
      ...
      return new Response(
        `Error converting metapage: ${error?.message || error}`,
        { status: 400 },
      );
    }
  },
          ...
          <div style="margin-bottom: 40px; margin-top: 20px; margin-left: 20px;">
            ...
            <div style="margin-left: 20px; background-color: #f0f0f0; padding: 10px; border-radius: 5px;">
              <code>
                ...
                {'\'{"version":"0.3","metaframes":{"r":{"url":"https://metapages.org/metaframes/random-data-generator/?frequency=1000"},"g":{"url":"https://metapages.org/metaframes/graph-dynamic/","inputs":[{"metaframe":"r","source":"y"}]}}}\''}
                {" "}
                {props.data.url}
              </code>
            </div>
