...
describe("Metapage secrets injection", async () => {
  ...
  const testDefinition: MetapageDefinition = await getMetapageDefinitionFromUrl(
  ...
  it("should remove secrets from definition events", async () => {
    ...
    let emittedDefinition: MetapageDefinition | null = null;
    ...
    if (emittedDefinition) {
      ...
      const defUrl = (emittedDefinition as MetapageDefinition).metaframes
        .secret1test.url;
      expect(
        getHashParamValueBase64DecodedFromUrl(defUrl, "secret1"),
      ).toBeUndefined();
  ...
  it("should preserve original URLs when there are existing hash params", async () => {
    const definitionWithHash: MetapageDefinition = {
      metaframes: {
        secret1test: {
  ...
  it("should inject secrets into multiple metaframes", async () => {
    ...
    const twoFrameDefinition: MetapageDefinition = {
      metaframes: {
        frame1: {
...
describe("Metapage secrets injection - integration test", () => {
  ...
  it("should preserve secrets when setDefinition is called with updated definition", async () => {
    const initialDefinition: MetapageDefinition = {
      metaframes: {
        secret1test: {
          url: "https://metapage.io/metaframes/passthrough-markdown/",
        },
    ...
    const updatedDefinition: MetapageDefinition = {
      metaframes: {
        secret1test: {
